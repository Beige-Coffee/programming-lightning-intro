...
The HTLC output is going to be a little gnarly. In fact, it won't be the same for Alice and Bob. Instead, it will change depending on if you are ***offering*** the HTLC or if you are ***recieving*** the HTLC. Head on over to the next page to learn how full HTLCs are implemented in Lightning!
