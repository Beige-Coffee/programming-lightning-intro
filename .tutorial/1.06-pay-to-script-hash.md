...
## Locking To Multisig
To build our intuition of a **P2WSH** script, let's walk through an example where we'd like to lock our funds such that multiple keys are required to spend those funds.
...
```
#### Question: Why is there a `0` in front of the first signature?
<details>
...
Rust Bitcoin provides a ```Builder``` object we can use to construct any ```Script``` we want. It offers a handful of helper functions for adding opcodes, ints, bytes, and keys to a script:
<br/>
