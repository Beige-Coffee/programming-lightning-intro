...
We've now seen how Alice and Bob can use a combination of an off-chain protocol and bitcoin transactions to build a payment channel between themselves. In this payment channel, Alice and Bob can trustlessly transfer funds between themselves by updating their channel balances. 
...
Utilizing what we've learned about so far, we can set up a way to trustlessly route payments across this network. Take a minute to think through how we can set up a new commitment transaction that does this. How will we represent this new output? Will it be included in the ```to_local``` or ```to_remote``` outputs, or will it be its own output?