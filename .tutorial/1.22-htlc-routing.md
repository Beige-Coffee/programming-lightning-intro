...
As you can see in the above picture, `OP_EQUAL` will return a `1` (True) or `2` (False) to the stack, depending on if the preimage that Bob provides is correct. In our prior script, this is perfect, since we want the script to end with a 1 if the preimage is correct. However, now that we've added an `OP_CHECKSIG`, we don't want a 1 left on the stack just yet. Instead, we'd simply like to continue script execution and proceed to the next step, which involves checking Bob's signature. For this reason, we use `OP_EQUALVERIFY`, which will fail the script if the preimage is incorrect and simply continue otherwise.
...
Imagine that Bob and Dianne's HTLC output expired at block height 200 - the same as Alice and Bob's. If this were the case, it would be possible for Dianne to wait until block 200 to claim the HTLC from Bob (remember, she knows the secret, because she generated it!). Simultaneously, Alice might assume the payment is going to expire, so she may reclaim her HTLC output at block height 200. If this were to happen, Dianne would have claimed the 400,000 sats from Bob, but he would **NOT** be able to claim his 405,000 sats from Alice. Ouch!

<p align="center" style="width: 50%; max-width: 300px;">
  <img src="./tutorial_images/dianne_htlc_claim.png" alt="dianne_htlc_claim" width="100%" height="auto">
</p>

To protect Bob against this sort of attack, Bob may decide that he requires, at least, a **20 block delay** to claim the HTLC from Alice. Therefore, he will create an HTLC that times out at **block height 180**. This ensures that, if Dianne provides the preimage right before the HTLC expires, Bob has **20 blocks** before his HTLC with Alice expires, giving him more time to claim his HTLC.

**FUN FACT**: This **delay** is actually called `cltv_expiry_delta`, and it's something that nodes in the Lightning announce to the network via the `channel_update` [message](https://github.com/lightning/bolts/blob/master/07-routing-gossip.md#the-channel_update-message)
...
To claim the HTLC, Dianne will have to publish the preimage **on-chain**! Recall, we just did this ourselves in the last exercise! Therefore, if Dianne did decide to close the channel at block height 180, Bob has until block height 200 to get the preimage **on-chain**, hand it to Alice, and claim his 405,000 sats.
...
All that said, let's briefly review the happy path. Under ideal circumstances, Dianne would simply send Bob the **preimage** via the `update_fulfill_htlc` [message](https://github.com/lightning/bolts/blob/master/02-peer-protocol.md#removing-an-htlc-update_fulfill_htlc-update_fail_htlc-and-update_fail_malformed_htlc), and they would work together to update their commitment transaction such that Dianne's output was 400,000 sats larger. This way, they would not have to close their channel by publishing an on-chain transaction. Once Bob receives the **preimage** from Dianne, he would work with Alice to update their channel state. Rememebr, the point of Lightning is to scale off-chain payments, so the goal is to *not* make on-chain payments.
