...
The above diagrams oversimplified the commitment transaction signatures, so it's worth expanding on them here.

When Alice and Bob agree to a new channel state, they will each have their own asymmetric version of the new commitment transaction. Since the transactions are slightly different, they require unique signatures from the counterparty. Therefore, when moving to a new channel state, Alice and Bob exchange signatures specifically for the counterparty's version of the commitment transaction.

To create the signature, the party proposing the update (ex: Alice) constructs the counterparty's (Bob's) version of the commitment transaction, generates her signature for it, and sends the signature to Bob. Bob then verifies the signature, constructs Alice's version, generates his signature for it, and sends it back to Alice. Even though Alice generates a version of Bob's commitment transaction to sign it, she cannot publish it without Bob's signature (which he will never provide). 
...
First, we'll need to calculate `h1` and `h2`. To calculate `h1`, we must obtain the SHA256 of the countersignatory revocation public key concatenated with the per-commitment public key (in that order).

We can calcuate `h2` by obtaining the SHA256 of the per-commitment public key concatenated with the countersignatory revocation public key (in that order).

A helper function has been provided for you, which you can use to hash to `PublicKey` types.
...
<details>
<summary>Step 2: Tweak the Countersignatory Revocation Public Key and Per Commitment Public Key</summary>

Once we've obtained `h1` and `h2`, we'll need to use them to tweak the Countersignatory Revocation Public Key and Per Commitment Public Key. Remember, tweaking a public key essentially means adding the public key to itself many times. A helper function, `pubkey_multipication_tweak`, has been provided for you.
