...
Note, this diagram does not go into the exact protocol details. Instead, it's meant to further build your intuition as to how the revocation keys work. In the appendix of this workbook, you'll find an optional section that digs into the nuances of how revocation keys work at the protocol level. 
...
### Step 1
At the start of Alice and Bob's Lightning channel, each part will generate the following sets of public and private keys:
- **Revocation Public Key and Private Key**: This is a public/private key pair that is constant across the entire length of a payment channel. As we'll soon see, this will be one piece that we'll use to create a new key pair for the penalty path.
- **Per-Commitment Public and Private Key**: For each commitment transaction, Alice and Bob will generate a new public/private key pair. This will be the other piece that we'll use to create the penalty path.

For this explanation, we're skipping over how these are generated. If you're interested in learning about that, please see the optional section in the appendix, titled "Revocation Keys Deep Dive". For now, it's sufficient to know that Alice and Bob will have have:
- A channel-specific public/private key pair, called "Revocation" keys.
- A commitment-specific public/private key pair, called "Per-Commitment" keys.
...
Once exchanged, each party can combine **their partner's Revocation Public Key** with **their Per-Commitment Public**, creating a new public key **that neither of them know the secret key to**. This is what they will use in the penalty spending path of their `to_local` output.

### Step 3
When Alice and Bob decide to advance to a new channel state, they will exchange the following keys:
- **The Prior State's Per-Commitment Private Key**
- **The Next State's Per-Commitment Public Key**

By doing this, each party provides **their counterparty** with the neccessary information (the prior state's per-commitment private key) to calculate the **their revocation public key**. In other words, Alice gives Bob the information needed to calculate Alice's **revocation public key** from the prior state. This effectively revokes Alice's old state, since Bob can now steal her funds if she publishes the old state.
