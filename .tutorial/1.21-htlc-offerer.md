...
Since the HTLC needs to be timelocked with an **absolute timelock** set to some block height in the future, the output cannot be spent until that block height has passed. This is meant to give Bob a specific timeframe for which he can retrieve the preimage and claim the funds.
...
3) This spending path requires signatures from both Alice and Bob to spend. They will pre-sign the HTLC Timeout transaction ahead of time, effectively both agreeing to the "expiry" terms of the contract. This last point is important, so you may want to read it again!
