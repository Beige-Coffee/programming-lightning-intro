...
**This is very nuanced and hard to see, but there is actually a bit of a dilema here!** Alice's spending path must be delayed by ```to_self_delay``` blocks so that Bob always has time to spend the funds via the recovation path if Alice cheats in the future. Remember, we enforce this delay using `OP_CHECKSEQUENCEVERIFY`, which is a **relative timelock** that only starts once the transaction is mined.
...
Additionally, the refund path must include a **relative timelock** to ensure that Alice must wait `to_self_delay` blocks to spend from the refund path. This path exists so that, if Alice decides to publish this transaction after her and Bob have agreed to move on, Bob will have time to punish her by spending from the revocation path.

This means, in the worst case, if Alice tried to spend this output after the contract expired, **she would still have to wait another** `to_self_delay` **blocks before she can spend the output**. This also gives Bob extra time to provide the preimage and spend this output to himself. Do you know how we can fix this?
...
Together, these changes allow for Alice to enforce the HTLC timeout while **also** enforcing the `to_self_delay`. This is because Alice can publish the **HTLC Timeout Transaction**, which effectively expires the HTLC, however, she cannot claim her funds until the `OP_CHECKSEQUENCEVERIFY` timelock is satisfied.
...
3) **Expiry Path**: If Bob doesn't provide the preimage, Alice can claim the output via the HTLC Timeout Transaction, **which is set up in advance with Bob's signature for the 2-of-2 multisig condition** (more on this below). This allows Alice to reclaim the funds after the `cltv_expiry`**. 
