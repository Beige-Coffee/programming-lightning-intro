...
## ⚡️ Get Our Updated Refund Transaction
...
Recall, since every commitment transaction has the same structure, including the refund transaction, we need to replace the **Tx Hex** and **Tx ID** for our original refund transaction in `src/ch1_intro_htlcs/transactions.txt`. Go ahead and make those updates by deleting the old information and pasting the new data.

Once this is done, we now have our refund transaction ready to broadcast, if needed, but it also has a revocation path for when we decide to move on to a new channel state. Nice!