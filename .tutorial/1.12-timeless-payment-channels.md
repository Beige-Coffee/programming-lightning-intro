...
<details>
  <summary>Answer</summary>
Changing the public key used in each commitment transaction will ensure that the transaction ID changes for each state, even if state is the same balance distribution.


For example, if Alice and Bob's commitment states look like the following, each transaction will have it's own unqiue transaction ID:
