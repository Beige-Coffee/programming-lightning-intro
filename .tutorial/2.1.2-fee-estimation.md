...
If you recall, when we close a channel cooperatively, both parties will close any pending HTLCs and create two outputs - one for each party. Both outputs will simply send the respecitve balance to each channel party, with no alternative spending paths. Therefore, it's safe to choose a low enough fee such that your transaction will enter the mempool and be mined eventually.
...
This one is a little tricky, since we have not discussed anchor outputs yet. In short, anchor outputs are additional outputs - one for each channel party - that lock bitcoin to the channel parties funding key. These outputs are just above the dust limit and are mainly meant to be used to fee-bump a commitment transaction via child-pays-for-parent. A child-pays-for-parent transaction is when there is one transaction that is sitting in the mempool, but it has a relatively low fee, and it probably won't be mined until fees subside. To speed up this transaction, you can create another transaction that spends from the output on the pending transaction, and put a very high fee on that second transaction. Since miners will have to include the parent transaction to be able to mine the second transaction (since the UTXO it's spending from doesn't exist until the parent transaction is mined), they will now include the parent transaction. 


This has implications for Lightning, since you have to decide on which fee to use *ahead of time* for commitment transactions that do not support anchor outputs. This is because, when you and your counterparty decide to move to a new channel state, you have to create the transactions with the new balance or HTLCs for that respective state. Therefore, you must also decide on which fees to set for this transaction. Since this transaction can, theoretically, be published at any time in the future (ex: your channel party goes offline), then you will need to guess which fee to use such that your transaction will be mined at some arbitrary point in the future.

Therefore, it's recommened to set a fee that is relatively conservative - such as a 12 block target.
</details>
...
## ⚡️ Implement `FeeEstimator` On Our `BitcoindClient`
As we've already done with the `BlockSource` and `BroadcasterInterface`, let's implement the FeeEstimator trait on our `BitcoindClient`. 

<p align="center" style="width: 50%; max-width: 300px;">
  <img src="./tutorial_images/node_setup/fee_estimation.png" alt="fee_estimation" width="90%" height="auto">
</p>

Bitcoin Core has an RPC command, `estimatesmartfee`, that esimates the **fee per kilobyte** needed for a transaction to be confirmed within a specified number of blocks, called the **confirmation target**. For this exercise, we'll simulate that RPC call by called `rpc_estimate_smart_fee`, This is a hard-coded function that is available to within our `BitcoindClient`. It's hard-coded because we don't have enough activity in our regtest environment for Bitcoin Core to be able to return an actual fee if we were to make that call.

In `src/ch2_setup/fee_estimator_exercise`, you'll find the below function. Per the LDK specs, this function will take a `ConfirmationTarget` and return the estimated fee for that `ConfirmationTarget`.
...
The below helper function is available to you within the `BitcoindClient`. You can access it by calling `self. rpc_estimate_smart_fee()`. It's also worth noting that Bitcoin Core's RPC command `estimatesmartfee` actually returns fees in BTC/KvB, which needs to converted to satoshis/KW. For this exercise, we abstract away that nuance, but, in reality, you can convert BTC/KvB to satoshis/KW by multiplying by 10^8 and then dividing by 4. This will give you the virtual-bytes into weight units
