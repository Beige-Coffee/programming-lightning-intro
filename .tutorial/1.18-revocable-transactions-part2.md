...
No, not really! If Alice attempts to publish an old transaction, it would become a race to see who can mine a new transaction that spends from this output. Even worse, since Alice is publishing the transaction, she has a head start.
...
When two channel partners decide to open a channel, they exchange some expectations as to how the channel will operate. For example, what the fees should be commitment transactions and how long the delayed spending path should be timelocked for.
...
Part of this depends on someone's risk preference. If they prefer to be very cautious, they will likely prefer that their counterparties use longer `to_self` delays so that they have more time to act. For example, if the channel balance is quite large, the users may prefer longer delays.
...
You may be tempted to think that Alice can just bump the fees using **Child Pays For Parent** (CPFP), but it's a little trickier than that. Alice's output has a relative timelock, **which doesn't start until the commitment transaction is mined**. Therefore, our CPFP transaction would be referencing an output that is **not yet spendable**, meaning our CPFP transaction cannot be included in a block until the timelock expires. Additionally, since many mempool policies restrict nodes from broadcasting transactions that have non-final timelock conditions, our CPFP transaction will not propogate through the network.

There are a few options to remedy this, and the chosen solution will depend on if our Lightning channel supports **anchor outputs** or (coming soon, as of Sep 2025!) **zero fee commitments**. If it does, then anchor outputs will enable us to fee-bump our commitment transaction at any point in time. Anchor outputs also have a crucial benefit of mitigating "pinning attacks". 

That said, while anchor outputs were certainly beneficial, they did not entirely alleviate concerns over fees. This is because, even with anchor outputs, channel partners still needed to do their best to estimate the *minimum feerate* that would be needed to get their transaction into the mempool and mined. The combination of [V3 TRUC Transactions](https://github.com/bitcoin/bips/blob/master/bip-0431.mediawiki), 1 Parent 1 Child (1P1C) package relay, and ephemeral anchors enabled zero-fee comittments, whereby channel parties do not have to add *any* fees to their commitment transactions. Instead, they can all be added when closing the channel - even in the case of a unilateral force close. 

With all that said, if our node does not support anchor outputs, then we will have to be very deliberate to select a feerate that is sufficiently high, thus reducing the probability of running into feerate issues if we have to broadcast a commitment transaction later. It's also worth nothing that this issue is most prevelant when our peer is offline or unreachable. If they are online and cooperative, we can work with them to update the fees.
...
Under the hood, `p2wpkh_output_script` simply takes a public key and converts it to a P2WPKH output.
