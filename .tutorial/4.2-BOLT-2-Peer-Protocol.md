...
1) A HashMap of all connected peers.
2) A `node_signer` (here, a `KeysManager`), which implements the `NodeSigner` trait for signing messages and transactions.
3) A secp_ctx, a Secp256k1 context used to create signatures for gossip messages, invoices, and more.

```rust
pub struct PeerManager{
  peers: HashMap<PublicKey, Peer>,
  node_signer: KeysManager,
  secp_ctx: Secp256k1<secp256k1::SignOnly>
}
```

### ⚡️ Complete `handle_open_channel` 
For this exercise, you’ll complete the `handle_open_channel` function in our `PeerManager`. This function handles incoming `open_channel` messages from other nodes, deciding whether to accept or reject the channel. It takes a counterparty’s node ID and an `OpenChannelMsg` as input and returns an `OpenChannelStatus` enum:
- `Reject` if the channel doesn’t meet our criteria
- `Accept` if it does

When our node receives an `open_channel` message, we need to perform checks to validate the channel. For simplicity, complete `handle_open_channel` to accept channels only if:
- `funding_satoshis` is at least 100,000 (ensures sufficient funds).
- `to_self_delay` is at most 144 (limits how long our funds can be locked).
- `commitment_feerate_sat_per_vbyte` is at least 10 sats/vByte (ensures sufficient fees).


These fields are available in the `OpenChannelMsg` type passed as `msg`. Access them like this:
```
...
impl PeerManager {
  ...
  fn handle_open_channel(&self, their_node_id: PublicKey, msg: &OpenChannelMsg) -> OpenChannelStatus {
