...
Before digging into the details, it's important to stress that each public key is unique for any given commitment transaction. To accomplish this, BOLT 3 provides specific guidance for how the **per commitment point** is created, ensuring it will be unique for each commitment state. That said, the **basepoint** will be the same across all commitment states.
...
pub(crate) fn new(seed: [u8; 32]) -> NodeKeysManager {
    ...
    // derive shutdown public key at index 1

    // derive channel master key (extended private key) at hardened index 2
