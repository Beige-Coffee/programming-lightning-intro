...
By now, you're probably itching to send some payments. After all, the real joy of Lightning is "slinging sats", as they say.
...
Lightning has a few different payment protocols - some offical and some unofficial.
### Official Payment Protocols
"Official" payment protocols are those that have a Lightning Basis of Technology (BOLT), which describes how they work. 
- **BOLT 11: Invoices**: BOLT 11 is the original payment protocol and, therefore, the most widely used and adopted as of early 2025. This protocol describes how one can *request* payments over lightning. This protocol is the one we'll implement in this section of the workshop.
...
### Unofficial Payment Protocols
"Unofficial" payment protocols are simply protocols that define a payment scheme but do not have a BOLT associated with them. They are often industry innovations that were created to service existing needs in the payment space.
...
- **Async Payments**: A down-side to most payment protocols on the Lightning network is that the recipient must be online when recieving the payment so that they can perform some action, such as releasing a preimage. Therefore, if they are offline when receiving the payment, it won't be able to complete. To remedy this, entreprenurs have developed asynchronous payments, whereby an intermediary routing node, such as a Lightning Service Provider (LSP), holds the Lightning payment until the recipient is online.
