...
Specifying an "up front shutdown script" can increase the security of Lightning channels. For example, if Bob is hacked and the hacker tries to initiatve a force close with a ***different*** address than what Bob originally specified, then Alice will refuse to cooperate.
