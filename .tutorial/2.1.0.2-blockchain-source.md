...
As we mentioned above, a Lightning Node needs to be able to retrieve blockchain data regardles of what the chain source is. Therefore, LDK has a provided a `BlockSouce` interface, which defines methods to retrieve block and header data from a blockchain source. The developer is in charge of implementing thse functions in whatever manner best suits their application's needs.

The `BlockSource` interface, with it's required methods, can be seen below. As you can see, LDK is essentially informing the developer that an LDK lightning node *must* provide functions to retrieve header data, block data, and the best block's hash. These functions will be called privately within LDK as part of our node's normal operations, but we must define them first.
...
Furthermore, we will encapsulate the `BlockSource` functionality within our `BitcoindClient` so that all of the Bitcoin Core interfaces are within the same structure. 

To do this, we use the bellow syntax:
...
```rust
let best_block = bitcoind_client.get_blockchain_info();
```
...
## Adding Additional Methods To `BitcoindClient`
...
Since we're using Bitcoin Core for our on-chain wallet, let's add the ability to fetch UTXOs to our `BitcoindClient`. To get a feel for what we're going to need to implement, go ahead and execute the below command in your **shell**.
...
## ⚡️ Implement `list_unspent` For Our `BitcoindClient`

For this exercise, we'll implement `list_unspent` for our `BitcoindClient`. To do this, we'll use another `impl` block in our Rust codebase, though, in reality, this would have likely been added underneath the `new` constructor we created previously.
...
Try to implement `list_unspent`. Note: you'll have to cast the response to a `ListUnspentResponse` type. Look to `get_blockchain_info` for guidance on how to do this.
