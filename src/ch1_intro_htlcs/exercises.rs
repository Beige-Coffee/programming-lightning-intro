...
pub fn build_refund_transaction(
    ...
    let alice_p2wpkh = p2wpkh_output_script(alice_pubkey);

    let bob_p2wpkh = p2wpkh_output_script(bob_pubkey);

    let alice_output = build_output(alice_balance, alice_p2wpkh);

    let bob_output = build_output(bob_balance, bob_p2wpkh);

    let version = Version::TWO;
    let locktime = LockTime::ZERO;

    let transaction = build_transaction(version, locktime, vec![funding_txin], vec![alice_output, bob_output]);
...
pub fn generate_revocation_pubkey(
    ...
    let R = tweak_pubkey(countersignatory_basepoint, h1);
    let P = tweak_pubkey(per_commitment_point, h2);

    let r = add_pubkeys(R, P);
...
pub fn build_commitment_transaction(
    ...
    let tx = build_transaction(version, locktime, vec![funding_txin],
...
pub fn build_htlc_commitment_transaction(
    ...
    let alice_output = build_output(local_amount, local_script.to_p2wsh());

    let bob_output = build_output(remote_amount, remote_script);
    ...
    let tx = build_transaction(version,
                      locktime,
                      vec![funding_txin],
                      vec![alice_output, bob_output, htlc_output]);
    tx
}
...
pub fn build_htlc_timeout_transaction(
    ...
    let output = build_output(htlc_amount, script.to_p2wsh());
